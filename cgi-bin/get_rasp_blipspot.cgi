...
use CGI::Carp qw(fatalsToBrowser);
  ...
  $EXTRACTSCRIPT = "/var/www/cgi-bin/extract.blipspot.PL";
  ...
  else
  {
    ...
    if ( defined $parameter && $parameter =~ m|^([A-Za-z0-9][A-Za-z0-9_ .-]*)$| ) {
       $parameter = $1 ;  # alphanumeric+
    }
    else {
       $parameter = '';
    }
    # PAULS if ( defined $plottime && $plottime =~ m|^([A-Za-z0-9][A-Za-z0-9_.-]*)$| ) { $plottime = $1 ; } # alphanumeric+
    if ( defined $plottime && $plottime =~ m|^([A-Za-z0-9][A-Za-z0-9_.-]*)$| )
	{ $plottime = $1 ; } # alphanumeric+
    else
	{$plottime = '' ; }
    if ( defined $linfo && $linfo =~ m|^([A-Za-z0-9][A-Za-z0-9_.-]*)$| ) { $linfo = $1 ; } # alphanumeric+
    ### if ( defined $linfo && $linfo =~ m|^([0-9+-][0-9]*)$| ) { $linfo = $1 ; } # integer only
  }

  #4testprint: `echo "ARGS: $REGION & $GRID & $DAY & $iimage & $ilat & $kimage & $klon & $imagewidth & $imageheight " >> /var/www/cgi-bin/LOG/get_rasp_blipspot.test_prints`;
  #PAULS `echo "ARGS: $REGION & $GRID & $DAY & $iimage & $ilat & $kimage & $klon & $imagewidth & $imageheight & $parameter " >> LOG/get_rasp_blipspot.test_prints`;
...
########################################
	...
	#4testprint: `echo "${EXTRACTSCRIPT} ARGS: $REGION & $GRID & $DAY & $igrid & $jgrid & $plottime & $parameter " >> /var/www/cgi-bin/LOG/get_rasp_blipspot.test_prints`;
